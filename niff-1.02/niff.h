/* $Header: /Cvsdata/www.music-notation.info/web/niffsdk/niff.h,v 1.1 2009/06/08 05:33:44 gerd Exp $ */
/*

	File:	NIFF.h
	Copyright:	None
	Version:	NIFF 6a* (with minor corrections,TWB 11 NOV 1995)
	Date:	November, 1995


This header includes typedefs for NIFF data types and tag, chunk, list and form structures.
Also, declarations for the four character list ID's and chunk ID's, and the
one character tag ID's.

A macro is defined, called MAKEFOURCC, which assembles four ascii characters into a four byte
DWORD variable.  Comment it out if you are using the Microsoft Multimedia I/O API.  This macro
assembles the four ascii characters into the DWORD in reverse order (i.e. from last to first).
The NIFFIO/RIFFIO Parser (see below) contains functions for reading FOURCC's from a NIFF file
and writing them to a NIFF file, so if you use this parser, the order of the characters should
be transparent to you.

You may also want to comment out the other typedefs which duplicate those in Windows,
such as BYTE, SIGNEDBYTE, DWORD, etc. 

* CORRECTIONS FROM NIFF 6a: 
	System Separation Mark chunk has its field named "where" (pg. 66).
    FONTIDX replaces FONTPTR (pg. 21).
    User chunk (described pg. 21) is defined here, but not in NIFF 6a chunk definitions.
    User tag (described pg. 21) is defined here, but not in NIFF 6a tag definitions.
    Midi Performance tag fields "startTime" and "duration" are LONG instead
    of RATIONAL
    Time Signature bottomNumber is now SIGNEDBYTE, instead of BYTE.

   MidiDataStream cannot have a variable length.
  
*** NIFFIO/RIFFIO Parser ***

This header may be used as part of the RIFFIO/NIFFIO public-domain parser developed
by Tim Butler. The parser source code contains high-level and low-level functions 
for reading and writing RIFF/NIFF files.  It was developed in part to help overcome 
the following programming challenges inherent in a multi-platform standard:

1. Integer values are always stored in a NIFF file with their
most significant byte first and their least significant byte last, like Motorola/Macintosh
machines, not Intel/IBM-compatible/Windows machines.

2.  In NIFF files, fields of type SHORT and LONG will always be exactly 16 and 32 bits,
respectively.  However, "short" and "long" in some C compilers may be longer.
Therefore, the structure members are sufficient for in-memory use,
but don't be misled into thinking they are neccessarily the same
size as the data in the file.  

3. Most compilers do some padding between fields within a structure 
for the purpose of byte alignment. This keeps you from being able to read
these structures in and write them out directly. 
...
typedef struct {
	SHORT	numerator;
	SHORT	denominator;
}						RATIONAL;		/*  two 2-byte signed integers, used for
			  								timing representation */
typedef long	STROFFSET;		/* a four byte signed integer pointing to a 
											RIFF ZSTR in the String Table chunk in the Setup 
											Section. */
typedef unsigned short 	FONTIDX;		/* a 2-byte index to a Font Description chunk in the
											Font Descriptions list. */
typedef char 			VARLEN[1];		/* used for fields of variable-length */

typedef struct {
	BYTE tagID;
	BYTE tagSize;				/* the size of the tag's data, beginning with <tagData> */
	BYTE tagData[1];			/* first byte of the tag's data */
} 						NIFF_TAG;

typedef struct {
	FOURCC 	ckID;
	DWORD 	ckSize;				/* the size of the chunk's data, beginning with <ckData> */
...
typedef struct {
	...
	DWORD 	formSize;				/* the size of the file, beginning with <formID> */
